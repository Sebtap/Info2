...
• Minuto \(N\): La cantidad de bacterias es exactamente el doble de la cantidad
 que había en el minuto anterior (\(N-1\)).
Realizar un programa en lenguaje C que contenga una función recursiva para 
calcular cuántas bacterias habrá en el laboratorio al cabo de un minuto \(N\) 
...
// 1. FUNCIÓN RECURSIVA PARA CALCULAR BACTERIAS
